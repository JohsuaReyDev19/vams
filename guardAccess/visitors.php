  ...
  <div class="container">
    ...
    <div class="grid">
      ...
      <div class="card scanner-card">
        ...
        <!-- Visitor Mode -->
        <div id="visitorMode" class="hidden1">
        ...
        <div id="otherVehicleInputWrapper" class="w-full hidden1">
          <input
            type="text"
          ...
          />
            ...
            <textarea id="visitorPurpose" class="scanner-input hidden" placeholder="Purpose of visit" rows="2" maxlength="100"></textarea>
      ...
      <div class="card">
        ...
        <div id="vehicleInfo">
          <div class="vehicle-info">
            ...
            <input type="text" id="accessName" value="<?php echo $_SESSION['full_name']; ?>" class="hidden">
      ...
      <div class="card">
          <div class="access-log">
            ...
            <div id="accessLogWrapper" style="max-height: 70vh; overflow-y: auto; padding-right: 0.5rem;">
...
<script>
  ...
  if (selected === "") {
    $("#vehicleTypeWrapper").addClass("hidden1");
    $("#otherVehicleInputWrapper").removeClass("hidden1");
    $("#customVehicleTypeInput").focus();
  } else {
    $("#vehicleTypeWrapper").removeClass("hidden1");
    $("#otherVehicleInputWrapper").addClass("hidden1");
  }
});
...
<script>
  $(document).on("click", "#rfidModeBtn", function () {
    $("#visitorMode").addClass('hidden1');
    $("#rfidMode").removeClass('hidden1');
    $("#rfidModeBtn").addClass("active");
    $("#visitorModeBtn").removeClass("active");
  });

  $(document).on("click", "#visitorModeBtn", function () {
    $("#visitorMode").removeClass('hidden1');
    $("#rfidMode").addClass('hidden1');
  ...
  $('#visitorForm').on('submit', function(e) {
    ...
    const data = {
      ...
      access: $('#accessName').val()
    };
    console.log(data.access);
    console.log(data.vehicle_type);

    $.post('../scanprocess/register_visitor.php', data, function(response) {
  ...
  $.get('../scanprocess/get_access_logs.php', function(data) {
...
function showLogDetails(log) {
  ...
  function searchVisitor() {
    ...
    fetch(`../scanprocess/search_visitor.php?plate=${encodeURIComponent(plate)}`)
        .then(res => res.json())
        .then(info => {
            if (info && info.plate) {
                ...
                document.getElementById("vehicleDetails").innerHTML = `
                    ...
                    <div class="vehicle-detail mt-3">
                        ...
                        <span>
                            ...
                            <button id="submitEntry" class="hidden1" onclick="submitEntry()" style="background-color:${buttonColor}; color:white; padding:0.4rem 1.2rem; border:none; border-radius:6px; cursor: pointer;">${action}</button>

                            <button id="submitExit" class="hidden1" onclick="submitExit(${info.log_id})" style="background-color:${buttonColor}; color:white; padding:0.4rem 1.2rem; border:none; border-radius:6px; cursor: pointer;">${action}</button>
                        </span>
                    </div>
                `;
...
const fullName = document.getElementById('accessName').value;
...
function submitEntry() {
    ...
    $.post('../scanprocess/entry_access_log.php', {
...
function submitExit(logId) {
  $.post('../scanprocess/update_time_out.php', { id: logId }, function(res) {
    if (res === 'success') {
