...
<body>
  ...
  <div class="p-6 sm:ml-64">
    <div class="w-full mx-auto" id="vehicleshow">
                    ...
                    <!-- Filters Section -->
                    <div class="w-full flex items-center flex-col md:flex-row md:items-center md:justify-between md:gap-4 space-y-2 md:space-y-0">
                    ...
                    <!-- Owner-Type Dropdown -->
                    <div class="w-full hidden md:w-52 flex items-center gap-1 hidden">
                        <label for="filter" class="font-medium text-gray-700 w-full md:w-auto">Filter: </label>
                        <select id="ownerTypeFilter"
                            class="w-full px-3 py-2 border border-gray-300 rounded-lg bg-white text-gray-700
                                focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-transparent shadow-sm text-sm">
                            <option value="Student">Student</option>
                            <option value="Faculty & Employee">Faculty & Employee</option>
...
?>
  ...
  <script>
    ...
    document.getElementById("ownerTypeFilter").addEventListener("change", function () {
        const selected = this.value.toLowerCase();

        const filtered = vehicles.filter(v => {
            if (selected === "") return true;  
            return v.owner_type.toLowerCase() === selected;
        });
...
// filter all the data from table
function filterVehicleList(searchTerm) {
    searchTerm = searchTerm.toLowerCase();

    const filtered = vehicles.filter(vehicle => {

        // Format date EXACTLY as shown in table → "Oct 20, 2025"
        let formattedDate = '';
        if (vehicle.registered_at) {
            ...
            formattedDate = `${monthShort} ${day}, ${year}`.toLowerCase();
        }

        // Combine all normal data + formatted date
        const allData =
            Object.values(vehicle).join(' ').toLowerCase() +
            ' ' + formattedDate;

        // Now search against EVERYTHING including formatted date
